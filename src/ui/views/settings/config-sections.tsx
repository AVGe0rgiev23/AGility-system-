...
import { ChoiceField, ConfigList, FlagField, NumberField, ReadOnlyField, RowActions, SettingsSection, TextField } from './config-controls'
...
function IndustriesEditor({ form, industries }: { form: ConfigFormView; industries: readonly string[] }) {
  ...
  return (
    <ConfigList form={form} path="industries" title="Industries" hint="A company's industry is picked from this list." addLabel="Add industry" onAdd={form.addIndustry}>
      <Table
        caption="Industries"
        columns={[
...
export function ConfigSections({ form, draft }: { form: ConfigFormView; draft: Config }) {
  return (
    <>
      <SettingsSection id="settings-agency" title="Agency">
      ...
      <SettingsSection id="settings-currency" title="Currency">
        <ReadOnlyField form={form} path="agencyCurrency" label="Agency currency" />
        ...
        <ReadOnlyField form={form} path="fxRates.rates.EUR" label="EUR per 1 EUR" />
      ...
      <SettingsSection id="settings-pricing" title="Pricing">
      ...
      <SettingsSection id="settings-estimation" title="Estimation">
      ...
      <SettingsSection id="settings-scoring" title="Scoring">
      ...
      <SettingsSection id="settings-roi" title="ROI">
      ...
      <SettingsSection id="settings-run-cost" title="Run costs">
        <RunCostDefaultsEditor form={form} items={draft.runCostDefaults} />
      </SettingsSection>

      <SettingsSection id="settings-storage" title="Storage">
        ...
        <ReadOnlyField form={form} path="storage.syncFolderHandleId" label="Folder handle" />
        <ReadOnlyField form={form} path="storage.lastSyncAt" label="Last sync" />
      </SettingsSection>

      <SettingsSection id="settings-ai" title="AI">
